...
def init_worker(pressure_levels_desc, geopotential_height_desc, temperature_desc, dew_point_temperature_desc, wind_direction_desc, wind_speed_desc, conv_params, accuracy):
    importr('thunder')
    ...
    attach_shared_mem(pressure_levels_desc, 'pressure_levels')
...
def process_pixel(coords):
    ...
    args_df = pd.DataFrame({
        "pressure": worker_data['pressure_levels'][hour, :, lat_idx, lon_idx],
...
def main():
    ...
    dew_point_temperature_desc = create_shared_np_array(dpt.m[:,::-1][:,1:])
    wind_direction_desc = create_shared_np_array(wdir[:,::-1][:,1:])
    wind_speed_desc = create_shared_np_array(ws[:,::-1][:,1:] * 1.984)

    pressure_levels_desc = create_shared_np_array(pres.values[:,::-1][:,1:] / 100)
    accuracy = 1
